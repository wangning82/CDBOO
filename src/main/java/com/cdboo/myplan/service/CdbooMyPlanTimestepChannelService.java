...
public class CdbooMyPlanTimestepChannelService
	...
	public void removeMyPlanTimestepChannel(CdbooMyPlanTimestep cdbooMyPlanTimestep){
	...
	@Transactional(readOnly = false)
	public void removeMyPlanTimestepChannel(CdbooMyPlan cdbooMyPlan) {

		/***********************
		 * 清空用户计划时段频道关联表数据，重新插入 Start
		 ***********************/
		CdbooMyPlanTimestep saveMyPlanTimestepObj = new CdbooMyPlanTimestep();
		saveMyPlanTimestepObj.setUserTimestep(new CdbooUserTimestep(cdbooMyPlan.getUserTimeStepId()));
		saveMyPlanTimestepObj.setPlan(cdbooMyPlan);
		List<CdbooMyPlanTimestep> list = cdbooMyPlanTimestepDao.findList(saveMyPlanTimestepObj);
		if (CollectionUtils.isNotEmpty(list)) {
			CdbooMyPlanTimestep cdbooMyPlanTimestep2 = list.get(0);
			CdbooMyPlanTimestepChannel delObj = new CdbooMyPlanTimestepChannel();
			delObj.setMyPlanTimestep(cdbooMyPlanTimestep2);
			super.delete(delObj);
		}
		/***********************
		 * 清空用户计划时段频道关联表数据，重新插入 End
	...
	public void saveTimestepChannel(CdbooMyPlan cdbooMyPlan) {
		...
		if (CollectionUtils.isNotEmpty(cdbooUserChannels)) {
			...
			if(CollectionUtils.isNotEmpty(list)){
				...
				/*********************** 清空用户计划时段频道关联表数据，重新插入 Start ***********************/
				CdbooMyPlanTimestep cdbooMyPlanTimestep2 = list.get(0);
				removeMyPlanTimestepChannel(cdbooMyPlanTimestep2);
				/*********************** 清空用户计划时段频道关联表数据，重新插入 End ***********************/
				...
				for (int i = 0; i < cdbooUserChannels.size(); i++) {
					...
					checkObj.setMyPlanTimestep(cdbooMyPlanTimestep2);
					...
					if (CollectionUtils.isNotEmpty(checkList)) {
						...
						myPlanTimestepChannel.setMyPlanTimestep(cdbooMyPlanTimestep2);
			...
			removeMyPlanTimestepChannel(cdbooMyPlan);
		}
	}
}
