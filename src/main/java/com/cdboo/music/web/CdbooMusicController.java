...
public class CdbooMusicController extends BaseController {
	...
	public String save(CdbooMusic cdbooMusic, Model model, RedirectAttributes redirectAttributes) {
		...
		try {
			cdbooMusicService.saveMusic(cdbooMusic);
			addMessage(redirectAttributes, "保存歌曲信息成功");
		} catch (Exception e) {
			e.printStackTrace();
			addMessage(redirectAttributes, "保存歌曲信息失败");
		}
	...
	@RequestMapping(value = "getMusicInfo", method=RequestMethod.POST)
	@ResponseBody
	public Map<String, Object> getMusicInfo(@RequestParam(required=true) String path) throws CannotReadException, IOException, TagException, ReadOnlyFileException, InvalidAudioFrameException{
		String musicPath = cdbooMusicService.getMusicPath(path);
		Map<String, Object> resolveMp3 = Mp3ResolveUtils.resolveMp3(musicPath);
		return resolveMp3;
	}
